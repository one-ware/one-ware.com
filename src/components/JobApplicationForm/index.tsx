import React, { useState, useEffect, useRef, ChangeEvent, FormEvent, DragEvent } from "react";
import axios from "axios";
import Translate, { translate } from "@docusaurus/Translate";
import { JobDetail, JobFormField } from "../../types/jobTypes";
...
const API_URL = process.env.NODE_ENV === "development"
  ...
  : "https://backend.one-ware.com/api/apply";
...
export default function JobApplicationForm({ job }: JobApplicationFormProps) {
  ...
  const initialFormData: FormData = {
    name: isDev ? "Test User" : "",
    email: isDev ? "daniel.ruppin@duobyte.dev" : "",
    github: isDev ? "one-ware" : "",
    website: "",
    resume: null,
  };
  ...
  const [submitStatus, setSubmitStatus] = useState<"idle" | "success" | "error" | "already_applied">("idle");
  ...
  const validateFile = (file: File): string | null => {
    if (file.type !== "application/pdf") {
      return translate({ id: "careers.form.error.fileType", message: "Please upload a PDF file" });
    }
    if (file.size > MAX_FILE_SIZE) {
      return translate({ id: "careers.form.error.fileSize", message: "File size must be less than 10MB" });
    }
    return null;
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      submitData.append("jobTitle", localizedJob.title);
      submitData.append("name", formData.name);
      submitData.append("email", formData.email);
      if (formData.github && formData.github.trim()) {
        let githubValue = formData.github.trim();
        if (githubValue.startsWith("https://github.com/") || githubValue.startsWith("http://github.com/")) {
          submitData.append("githubUrl", githubValue);
        } else if (githubValue.startsWith("github.com/")) {
          submitData.append("githubUrl", `https://${githubValue}`);
        } else {
          submitData.append("githubUrl", `https://github.com/${githubValue}`);
        }
      }
      ...
      if (response.data.success) {
        ...
        setSubmitStatus("already_applied");
      } else {
  ...
  const getLocalizedLabel = (field: JobFormField): string => {
    const isGerman = typeof window !== "undefined" && window.location.pathname.startsWith("/de");
    if (isGerman && field.label_de) {
      return field.label_de;
    }
    return field.label;
  };

  const getLocalizedOptions = (field: JobFormField): string[] => {
    const isGerman = typeof window !== "undefined" && window.location.pathname.startsWith("/de");
    if (isGerman && field.options_de && field.options_de.length === field.options?.length) {
      ...
      <div className="relative p-8 md:p-12 rounded-2xl" style={{ background: "linear-gradient(135deg, rgba(0, 255, 209, 0.1), rgba(0, 255, 209, 0.05))", border: "1px solid rgba(0, 255, 209, 0.2)" }}>
        <div className="text-center py-8">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">
            <Translate id="careers.form.success.title">Application Submitted!</Translate>
          </h3>
          <p className="text-gray-400 text-lg max-w-md mx-auto">
            <Translate id="careers.form.success.message">
              ...
              <div className="space-y-3">
                ...
                {localizedJob.startDate && (
                  <div className="flex items-center gap-3 text-white">
                    ...
                    <span>{localizedJob.startDate}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="lg:w-[55%]">
            <form onSubmit={handleSubmit} className="space-y-4">
              <input
                type="text"
                name="website"
                value={typeof formData.website === "string" ? formData.website : ""}
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  placeholder={translate({ id: "careers.form.name", message: "Name" }) + " *"}
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  required
                  placeholder={translate({ id: "careers.form.email", message: "E-Mail" }) + " *"}
                  className="w-full pl-12 pr-4 py-3.5 bg-white/[0.03] border border-white/10 rounded-xl text-white placeholder-white/50 outline-none focus:bg-white/[0.05] focus:border-[var(--ifm-color-primary)] focus:shadow-[0_0_0_3px_rgba(0,202,165,0.15)] transition-all duration-200"
                />
              </div>
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder={translate({ id: "careers.form.github", message: "GitHub Username" })}
                  className="w-full pl-12 pr-4 py-3.5 bg-white/[0.03] border border-white/10 rounded-xl text-white placeholder-white/50 outline-none focus:bg-white/[0.05] focus:border-[var(--ifm-color-primary)] focus:shadow-[0_0_0_3px_rgba(0,202,165,0.15)] transition-all duration-200"
                />
              </div>
              ...
              <div>
                <div
                  ...
                  className={`relative cursor-pointer rounded-xl border-2 border-dashed p-6 text-center transition-all duration-200 ${
                    fieldErrors.has("resume")
                      ? "border-red-500/50 bg-red-500/10"
                      : isDragging
                        ? "border-[var(--ifm-color-primary)] bg-[var(--ifm-color-primary)]/10"
                        : formData.resume
                          ? "border-[var(--ifm-color-primary)]/50 bg-[var(--ifm-color-primary)]/5"
                  ...
                  {formData.resume instanceof File ? (
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-white/50 text-sm">{(formData.resume.size / 1024 / 1024).toFixed(2)} MB</p>
                      </div>
                      <button
                        type="button"
                        onClick={(e) => {
                    ...
                    <>
                      ...
                      <p className="text-white/70 mb-1">
                        <Translate id="careers.form.resume.drag">Drag & drop your resume here or click to browse</Translate>
                      </p>
                      <p className="text-white/50 text-sm">
                        <Translate id="careers.form.resume.format">PDF only, max 10MB</Translate> *
                      </p>
                    </>
                  )}
                </div>
                ...
                {fieldErrors.has("resume") && !fileError && (
                  <p className="mt-2 text-red-400 text-sm">
                    <Translate id="careers.form.error.resumeRequired">Please upload your resume</Translate>
                  </p>
                )}
              </div>

              {job.formFields && job.formFields.map((field) => (
                    ...
                    <div ref={(el) => { dropdownRefs.current[field.name] = el; }} className="relative">
                      ...
                      >
                        <span className={formData[field.name] ? "text-white" : "text-white/50"}>
                          {formData[field.name]
                            ? getLocalizedOptions(field)[field.options.indexOf(formData[field.name] as string)] || String(formData[field.name])
              ...
              {submitStatus === "already_applied" && (
                <div className="flex items-center gap-3 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-yellow-300 text-sm">
                  ...
                  <Translate id="careers.form.alreadyApplied">
              ...
              {submitStatus === "error" && (
                <div className="flex items-center gap-3 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-300 text-sm">
                  ...
                  <Translate id="careers.form.error">
                    An error occurred. Please try again or contact us at career@one-ware.com.
                  </Translate>
                </div>
              )}
                ...
                <span className="relative z-10 text-[var(--ifm-color-primary)] group-hover:text-[var(--ifm-color-primary-lighter)] transition-colors">
                  {isSubmitting ? (
                    <span className="flex items-center justify-center gap-2">
                      ...
                      <Translate id="careers.form.submitting">Submitting...</Translate>
                    </span>
                  ) : (
                    <Translate id="careers.form.submit">Submit Application</Translate>
                  )}
                </span>
                <div
              ...
              <p className="text-xs text-white/50">
                <Translate id="careers.form.privacy">By continuing, you are indicating that you accept our</Translate>{" "}
                <a href="/docs/contact/privacy" className="text-[var(--ifm-color-primary)] hover:underline">
                  <Translate id="careers.form.privacyLink">Privacy policy</Translate>
                </a>
              </p>
            </form>
          </div>
