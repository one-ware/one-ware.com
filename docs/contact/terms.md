...
5.4. **AI Agent Inputs (Prompts)**  
Inputs made by the customer to the AI Assistant ("Prompts") are also considered Content within the meaning of this Section 5. The customer is prohibited from entering personal data or illegal content into the AI Agent. The customer bears full responsibility for the content of their prompts.
...
5.1. **Liability for AI Outputs**: The outputs generated by the AI Agent (e.g., configuration files, code snippets, technical answers) are automated and non-binding. ONE WARE assumes no liability for the correctness, completeness, or suitability of these outputs for a specific purpose. The use of AI-generated results, especially productive use of configuration files, is at the customer's own risk. ONE WARE generally does not owe any specific success with regard to AI advice.
6. Liability for damages due to data loss is limited to the amount of the restoration of the data that would have been incurred even if the customer had backed up the data regularly and in a manner appropriate to the risk. 
...
- **Storage**: Questions and answers are stored to enable history and context.  
- **Disclaimer**: The AI Agent generates answers based on probabilities. There is no claim to completeness, freedom from errors, or suitability for a specific purpose. The customer is responsible for technically validating the results before use.
