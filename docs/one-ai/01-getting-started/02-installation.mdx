...
If you want to create an AI model for a specific hardware, ONE AI needs to know the technical details of that hardware to optimize the model for the available resources. You can [define the hardware resources yourself](/docs/one-ai/documentation/hardware-settings), but we also offer existing configurations for a growing list of hardware. You can install additional configuration files in the ``Extension Manager`` under ``Hardware`` > ``OneAI Support``.
